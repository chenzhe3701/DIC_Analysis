...
% in classification.
%
% chenzhe, 2018-05-01.  I moved all the newly defined normalized scores to
% a new script_6_..._normalized.  So, this code was cleaned to keep only
% the scores in the old definition.
...
%% (1.1) Use [StrainScore (c=7)], set (phi_th=-0.22)
...
%% (1.2) Use [StrainScore (c=7)], set (phi_th=-0.17)
...
%% (1.3) use [ShapeScore], set (eta_th=0.2)
strainScoreCF = -0.22;    % 0.22, 0.17
shapeScoreCF = 0.2;
...
%% (1.4) use both score [StrainScore(c=7)]OR[ShapeScore], set (phi_th=-0.22)OR(eta_th=0.2)
strainScoreCF = -0.22;    % 0.22, 0.17
shapeScoreCF = 0.2;
% previous criterion, 7Dis+(0.5-SF)< scoreCF
% i.e., new criterion, 7Dis-SF<socre-0.5
...
%% (3.1) [ACC] vs. [StrainScore_th] alone (C=7) , compare at [ALL] strain levels
close all;
    ...
    for iS = 1:length(stru)
        for iCluster = 1:length(stru(iS).cLabel)
            ...
            info = [info; stru(iS).trueTwin(iCluster),...
                C*stru(iS).dis(iCluster)-stru(iS).sf(iCluster),...
                stru(iS).cvInc(iCluster)*stru(iS).tProbMax(iCluster),  1];
        end
    end
    
    
    % sort rows, based on col-2(strainScore,c=7)    
...
%% (3.2) [ACC] vs. [ShapeScore_th] alone, compare at [ALL] strain levels
clear scoreSorted ACC ACC_max F1 F1_max bestScore TP_best FP_best FN_best TN_best PPV_best TPR_best;
figure; hold on;
colors = lines(7);
colors([2,3],:)=colors([3,2],:);
...
%% (3.3) [ACC] vs. [StrainScore_th] alone (C=Fitted) , compare at [ALL] strain levels
clear scoreSorted ACC ACC_max F1 F1_max bestScore TP_best FP_best FN_best TN_best PPV_best TPR_best;
figure; hold on;
colors = lines(7);
colors([2,3],:)=colors([3,2],:);
...
for iE=iE_start:iE_stop
    ...
    % sort rows, based on col-2(strainScore,c=fitted)    
    info = sortrows(info,2);
    scoreSorted{iE} = info(:,2);
...
close all;
%% (4.1 a)  Just look at [iE=4], how [Counts] (TP,FP,FN,TN) changes with [StrainScore_th] (c=7)   
...
%%
print(['iE=',num2str(iE),' Event Count vs StrainScore_th.tif'],'-dtiff');
...
%%
print(['iE=',num2str(iE),' Event Count vs StrainScore_th zoom.tif'],'-dtiff');


%% (4.1 c) how [StrainScore_th] affect [ACC,precision,sensitivity], precision = PPV = TP/TP+FP, and sensitivity TPR=TP/TP+FN   
figure; hold on;
...
%% (4.2 a)  Just look at [iE=4], [how Counts] (TP,FP,FN,TN) changes with [ShapeScore_th)   
...
%%
print(['iE=',num2str(iE),' Event Count vs ShapeScore_th.tif'],'-dtiff');
...
%%
print(['iE=',num2str(iE),' Event Count vs ShapeScore_th zoom.tif'],'-dtiff');
...
%% (4.3 a)  Just look at [iE=4], how [Counts] (TP,FP,FN,TN) changes with [StrainScore_th] (c=fitted) 
clear scoreSorted ACC ACC_max F1 F1_max bestScore TP_best FP_best FN_best PPV_best TPR_best;
figure; hold on;
colors = lines(7);
...
rectangle('Position',[-0.5 0 0.5 600],'edgecolor','k','linewidth',2)
%%
print(['iE=',num2str(iE),' Event Count vs StrainScore_th C_fitted.tif'],'-dtiff');
...
set(gca,'xlim',[-0.5 0],'ylim',[0 600],'fontsize',18)
%%
print(['iE=',num2str(iE),' Event Count vs StrainScore_th C_fitted zoom.tif'],'-dtiff');
