...
fName_c2t_result = [sampleName,'_s',num2str(STOP{iE+B}),'_cluster_to_twin_result.mat'];
load([saveDataPath,fName_c2t_result],'stru','clusterNumMap','clusterNumMapCleaned','strainScoreMap','shapeScoreMap','trueTwinMap','scoreCF','cVolGrowthRatioMap','tProbMaxMap');

%% (1.1) Use [phi_StrainScore (c=7)], set (phi_th=-0.22)
strainScoreCF = -0.22;    % 0.22, 0.17
shapeScoreCF = 0.2;
...
ttl = ['StrainScore-th=',num2str(strainScoreCF)];
...
%% (1.2) Use [phi_StrainScore (c=7)], set (phi_th=-0.17)
strainScoreCF = -0.17;    % 0.22, 0.17
shapeScoreCF = 0.2;
% previous criterion, 7Dis+(0.5-SF)< scoreCF
% i.e., new criterion, 7Dis-SF<socre-0.5
...
%% (1.3) use [eta_ShapeScore], set (eta_th=0.2)
strainScoreCF = -0.22;    % 0.22, 0.17
shapeScoreCF = 0.2;
...
% set(a,'fontsize',18,'xticklabel',{''},'yticklabel',{''});
set(a,'fontsize',18);
ttl = ['ShapeScore-th=',num2str(shapeScoreCF)];
...
%% (1.4) use both score (c=7), set (phi_th=-0.22)OR(eta_th=0.2)
strainScoreCF = -0.22;    % 0.22, 0.17
shapeScoreCF = 0.2;
% previous criterion, 7Dis+(0.5-SF)< scoreCF
% i.e., new criterion, 7Dis-SF<socre-0.5
...
% set(a,'fontsize',18,'xticklabel',{''},'yticklabel',{''});
set(a,'fontsize',18);
ttl = ['(StrainScore-th=',num2str(strainScoreCF),') OR (ShapeScore-th=',num2str(shapeScoreCF),')'];
...
%% (1.5) use [new eta_ShapeScore normalized], set (eta_th=0.55)
strainScoreCF = -0.22;    % 0.22, 0.17
shapeScoreCF = 0.5;
...
% set(a,'fontsize',18,'xticklabel',{''},'yticklabel',{''});
set(a,'fontsize',18);
ttl = ['ShapeScoreNew-th=',num2str(shapeScoreCF)];
ttl_text = ['\eta_{th}= ',num2str(shapeScoreCF)];
title(a,ttl,'fontweight','normal');
annotation(f,'textbox', [0.635 0.96 0.3 0.042], 'String',ttl_text, 'LineStyle','none', 'FontSize',24);
...
%% (1.6) use both score [StrainScore (c=7)] OR [ShapeScore_New], set (phi_th=-0.22)OR(eta_th=0.55)
strainScoreCF = -0.22;    % 0.22, 0.17
shapeScoreCF = 0.55;
% previous criterion, 7Dis+(0.5-SF)< scoreCF
% i.e., new criterion, 7Dis-SF<socre-0.5
...
% set(a,'fontsize',18,'xticklabel',{''},'yticklabel',{''});
set(a,'fontsize',18);
ttl = ['(StrainScore-th=',num2str(strainScoreCF),') OR (ShapeScoreNew-th=',num2str(shapeScoreCF),')'];
ttl_text = ['(\phi_{th}= ',num2str(strainScoreCF),') OR (\eta_{th}= ',num2str(shapeScoreCF),')'];
title(a,ttl,'fontweight','normal');
annotation(f,'textbox', [0.635 0.96 0.3 0.042], 'String',ttl_text, 'LineStyle','none', 'FontSize',24);
%% maximize plot and run this:
...
% (3.1) using strainScore alone (C=7) , compare [ALL] strain levels
close all;
...
for iE=iE_start:iE_stop
    ...
    for iS = 1:length(stru)
        for iCluster = 1:length(stru(iS).cLabel)
            % [trueTwin, strainScore, shapeScore,     newStrainScoreBasedOnC, (newShapeScore=cVolGrowthRatio+1)/2*tProbMax, dummy=1, ...]
            info = [info;
                stru(iS).trueTwin(iCluster), stru(iS).strainScore(iCluster), stru(iS).shapeScore(iCluster),...
                C*stru(iS).dis(iCluster)-stru(iS).sf(iCluster), (stru(iS).cVolGrowthRatio(iCluster,iE)+1)/2*stru(iS).tProbMax(iCluster) 1];
        end
    end
    
    
    % sort rows, based on col-2(strainScore,c=7) or col-4(strainScore,c=fitted)   
...
%%
print(['Event Proportion vs StrainScore_th c_7.tif'],'-dtiff');
...
for iE=iE_start:iE_stop
    ...
    % sort rows, based on col-3(shapeScore) or col-5(NewShapeScore-normalized) 
...
%% (3.3) using strainScore(c=fitted) alone, compare [ALL] strain levels
clear scoreSorted ACC ACC_max F1 F1_max bestScore TP_best FP_best FN_best TN_best PPV_best TPR_best;
figure; hold on;
colors = lines(7);
colors([2,3],:)=colors([3,2],:);
...
for iE=iE_start:iE_stop
    ...
    % sort rows, based on col-2(strainScore,c=7) or col-4(strainScore,c=fitted)   
    info = sortrows(info,4);
    scoreSorted{iE} = info(:,4);
...
t1 = table(bestScore(iE_start:iE_stop)',...
    ...
    'VariableNames',{'StrainScore_th','Accuracy','TP','FP','FN','TN','Precision','Sensitivity'})
%%
print(['Event Proportion vs StrainScore_th C_fitted.tif'],'-dtiff');

%% (3.4) using shapeScore (new normalized) alone, compare [ALL] strain levels
clear scoreSorted ACC ACC_max F1 F1_max bestScore TP_best FP_best FN_best TN_best PPV_best TPR_best;
figure; hold on;
colors = lines(7);
colors([2,3],:)=colors([3,2],:);
...
for iE=iE_start:iE_stop
    ...
    % sort rows, based on col-3(shapeScore) or col-5(NewShapeScore-normalized) 
    info = sortrows(info,5,'descend');
    scoreSorted{iE} = info(:,5);
...
ylabel('ACC');
xlabel('\eta_{th}'); % xlabel('ShapeScore Threshold');
set(gca,'fontsize',18,'xlim',[-0.1, 1]);
legend({'strain level 2: -0.6%','strain level 3: -1.2%','strain level 4: -2.1%','strain level 5: -3.7%'},'location','best');
legend({'\fontsize{24}\epsilon\fontsize{16}^G = -0.004','\fontsize{24}\epsilon\fontsize{16}^G = -0.012','\fontsize{24}\epsilon\fontsize{16}^G = -0.023','\fontsize{24}\epsilon\fontsize{16}^G = -0.039'},'location','best');
...
t1 = table(bestScore(iE_start:iE_stop)',...
    ...
    'VariableNames',{'ShapeScore_th','Accuracy','TP','FP','FN','TN','Precision','Sensitivity'})
%%
print(['Event Proportion vs ShapeScore_th eta_normalized.tif'],'-dtiff');


%% (4) For selected iE, Look at metrics change with threshoulds
close all;
% (4.1 a)  Just look at iE=4, how TP, FP, FN, TN changes with threshold of StrainScore (c = 7)   
...
% sort rows, based on col-2(shapeScore)
info = sortrows(info,2);
scoreSorted = info(:,2);
...
%%
print(['iE=',num2str(iE),' Events vs StrainScore_th.tif'],'-dtiff');
...
%%
print(['iE=',num2str(iE),' Events vs StrainScore_th zoom.tif'],'-dtiff');


%% (4.1 c) how threshold of StrainScore affect ACC, precision = PPV = TP/TP+FP, and sensitivity TPR=TP/TP+FN   
figure; hold on;
...
%%
print(['iE=',num2str(iE),' Event Proportion vs StrainScore_th.tif'],'-dtiff');


%% (4.2 a)  Just look at iE=4, how TP, FP, FN changes with threshold of ShapeScore   
...
% sort rows, based on col-3(shapeScore) or col-5(new shapeScore normalized)   
info = sortrows(info,3,'descend');
scoreSorted = info(:,3);
...
%%
print(['iE=',num2str(iE),' Events vs ShapeScore_th.tif'],'-dtiff');
...
%%
print(['iE=',num2str(iE),' Events vs ShapeScore_th zoom.tif'],'-dtiff');


%% (4.2 c) how threshold of ShapeScore affect precision = PPV = TP/TP+FP, and sensitivity TPR=TP/TP+FN   
figure; hold on;
plot(scoreSorted, ACC,'linewidth',3,'color',[0 0 0]);
plot(scoreSorted, PPV,'linewidth',1.5,'color',[1 0 0]);
plot(scoreSorted, TPR,'linewidth',1.5,'color',[0 0 1]);
...
legend({'Accuracy (ACC)','Precision (PPV)','Sensitivity (TPR)'},'location','best');
%%
print(['iE=',num2str(iE),' Event Proportion vs ShapeScore_th.tif'],'-dtiff');


%% (4.3 a)  Just look at iE=4, how TP, FP, FN, TN changes with threshold of StrainScore (c=fitted) 
clear scoreSorted ACC ACC_max F1 F1_max bestScore TP_best FP_best FN_best PPV_best TPR_best;
figure; hold on;
colors = lines(7);
...
% sort rows, based on col-2(shapeScore c=fitted)
info = sortrows(info,4);
scoreSorted = info(:,4);
...
rectangle('Position',[-0.5 0 0.5 600],'edgecolor','k','linewidth',2)
%%
print(['iE=',num2str(iE),' Events vs StrainScore_th C_fitted.tif'],'-dtiff');
...
set(gca,'xlim',[-0.5 0],'ylim',[0 600],'fontsize',18)
%%
print(['iE=',num2str(iE),' Events vs StrainScore_th C_fitted zoom.tif'],'-dtiff');


%% (4.3 c) how threshold of StrainScore affect ACC, precision = PPV = TP/TP+FP, and sensitivity TPR=TP/TP+FN   
figure; hold on;
plot(scoreSorted, ACC,'linewidth',3,'color',[0 0 0]);
plot(scoreSorted, PPV,'linewidth',1.5,'color',[1 0 0]);
plot(scoreSorted, TPR,'linewidth',1.5,'color',[0 0 1]);
...
legend({'Accuracy (ACC)','Precision (PPV)','Sensitivity (TPR)'},'location','best');
%%
print(['iE=',num2str(iE),' Event Proportion vs StrainScore_th C_fitted.tif'],'-dtiff');


%% (4.4 a)  Just look at iE=4, how TP, FP, FN changes with threshold of StrainScore (new, normalized)   
clear scoreSorted F1 F1_max bestScore TP_best FP_best FN_best PPV_best TPR_best;
figure; hold on;
colors = lines(7);
...
rectangle('Position',[0.4 0 0.4 600],'edgecolor','k','linewidth',2)
%%
print(['iE=',num2str(iE),' Events vs ShapeScore_th eta_normalized.tif'],'-dtiff');
...
plot(scoreSorted, FN,'linewidth',2,'color',[0 0 1]);
plot(scoreSorted, TN,'--k','linewidth',1.5);
set(gca,'fontsize',18,'xlim',[0.4, 0.8],'ylim',[0 600])
%%
print(['iE=',num2str(iE),' Events vs ShapeScore_th eta_normalized zoom.tif'],'-dtiff');


%% (4.4 c) how threshold of ShapeScore affect precision = PPV = TP/TP+FP, and sensitivity TPR=TP/TP+FN   
figure; hold on;
plot(scoreSorted, ACC,'linewidth',3,'color',[0 0 0]);
plot(scoreSorted, PPV,'linewidth',1.5,'color',[1 0 0]);
plot(scoreSorted, TPR,'linewidth',1.5,'color',[0 0 1]);
...
legend({'Accuracy (ACC)','Precision (PPV)','Sensitivity (TPR)'},'location','best');
%%
print(['iE=',num2str(iE),' Event Proportion vs ShapeScore_th eta_normalized.tif'],'-dtiff');
