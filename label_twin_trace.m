...
function [fragments, struCell, haveActiveSS] = label_twin_trace(cluster_number_maps_cleaned,x_local,y_local, indR_min,indR_max, indC_min,indC_max, ID_local,ID_current,...
    struCell,iS,iE,iC,iE_list,iC_list,iEC,traceND,traceSF,sampleMaterial,twinTF,debugTF,th_1,th_2)
...
% check if need to go back.  If have activeSS, and not the first in the iE_list, go back
while (haveActiveSS)&&(iEC~=1)
    display(iEC);
    [fragments, struCell, haveActiveSS] = label_twin_trace(cluster_number_maps_cleaned,x_local,y_local, indR_min,indR_max, indC_min,indC_max, ID_local,ID_current,...
        struCell,iS,iE_list(iEC-1),iC_list(iEC-1),iE_list,iC_list,iEC-1,traceND,traceSF,sampleMaterial,twinTF,debugTF, 0.25, 0.7);
end
