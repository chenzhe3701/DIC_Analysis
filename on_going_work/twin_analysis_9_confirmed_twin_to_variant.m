...
% Step-4 and step-6 are actually for identifying twin-gb intersection.
% It's better to do this after cleaning up the twin variant map.
% Therefore, these two steps should not need to be run here.
...
saveDataPath = [uigetdir('D:\WE43_T6_C1\Analysis_by_Matlab_reducedAOI','choose a path of the saved processed data, or WS, or etc.'),'\'];
...
% (0) load data, to a matfile, that contains [clusterNumMapCleaned]  
                    ...
                    if struCell{iE}(iS).cTrueTwin(iC,iTwin)==1
                        ...
                        for ir = 1:nr
                            if any(vMap_r(ir,:))
                                ...
                                gbL = uniqueBoundary_r(ir,icL_back);    % gbL: label# for left gb
                                icR_back = find(uniqueBoundary_r(ir,:),1,'last');
                                gbR = uniqueBoundary_r(ir,icR_back);    % gbR: label# for right gb
                                gbLR(ir,:) = [gbL, gbR];
                                ...
                                while (icL_front<=icR_front)
                                    ...
                                    if (icL_front-icL_back)<=(icR_back-icR_front)
                                        ...
                                        while(vMap_r(ir,icL_front))
                                            gbLabelMap(ir,icL_front) = 1;   % prepare to assign label, temporarily make it = 1
                                        ...
                                        gbLabelMap(gbLabelMap==1) = gbL;    % change the value to label#, so that '1' can be used again as temp label for next row
...
%                         % (Step-5) go back to clean.  Sometimes, no intersection is determined for a row, so that row on gbLabelMap need to change back to 0.    
%                         cleanTF = 1;
...
%                         % end of (step-5)
                        ...
                        % (step-6) update cslMap for this twin
                        temp = cslMap;
...
for iE = iE_start:iE_stop
    ...
    trueTwinMapCell{iE}(trueTwinMapCell{iE}>0) = trueTwinMapCell{iE}(trueTwinMapCell{iE}>0);    % use variant number (1-6) instead of it + 18
