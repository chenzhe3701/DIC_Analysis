...
%
% chenzhe, 2020-04-20 note
% This code basically read the line that labels twin-gb intersection from a
% struRef, and use it as if it is drawn manually, as a input to label
% twin-gb intersection, and record results in struCell.
% 
% Looks like when I develop this code, some lines were draw with
% redundancy. So there were some codes added to check the distance:
% if all(cellfun(@(x) sum(abs(x(:)-line_t(:)))>50, lines_previous_iE))
...
% For struCell, trueTwinMapCell
[confirmedLabelFile, confirmedLabelPath] = uigetfile('D:\WE43_T6_C1\Analysis_2021_09\WE43_T6_C1_final_variant_map.mat','select the file for struCell and trueTwinMapCell');

% Load reference, if want to used previous label instead of manually draw line  
[refFile, refPath] = uigetfile('D:\WE43_T6_C1\Analysis_2021_09\possibly useful data\20191102_2147_twin_at_boundary_manual_result.mat','select the results of analyzing twin-grain boundary intersection');
...
% load struCell with previous twin_gb interaction result, for reference.
load(fullfile(refPath, refFile),'struCell');
...
%                                 [~,iTwin] = min(abs(direction-traceDir));  
                                ...
                                if ang_min < 20
                                    ...
                                    warning('drawn line not similar to any active trace');
                                end
                            case 'Redraw'
...
save(fullfile(saveDataPath, [timeStr,'_twin_gb_intersection_manual_label']), 'TA', 'tb_gbNum', 'tb_iE', 'tb_gNum', 'tb_tsNum', 'tb_pts', 'tb_iE_inGrain', 'tb_iE_atBoundary',...
    'tBoundaryCell','struCell','-v7.3');
try
    save(fullfile(saveDataPath, [timeStr,'_twin_gb_intersection_manual_label.mat']), 'tBoundary_accum','-v7.3');
catch
    error('no tBoundary_accum data, based on processing history');
end
% old name: '..._twin_at_boundary_manual_result.mat';



%% 2020-04-19 Check if the new struCell is the same as the struRef
% (1) they should be the same
% (2) looks like when I develop this code, some lines were draw with
% redundancy. So there were some codes added to check the distance:
% if all(cellfun(@(x) sum(abs(x(:)-line_t(:)))>50, lines_previous_iE))
warning on;
for iE = 2:5
   for iS = 1:length(struCell{iE})
      for iTwin = 1:6
         % compare s.tGb
         if ~isequal(struRef{iE}(iS).tGb{iTwin}, struCell{iE}(iS).tGb{iTwin})
             warning('tGb not equal');
         end
         
         for iGb = 1:length(struRef{iE}(iS).tGb{iTwin})
             if length(struCell{iE}(iS).tGbPts{iTwin})>=iGb
                 if ~isequal(struRef{iE}(iS).tGbPts{iTwin}{iGb}, struCell{iE}(iS).tGbPts{iTwin}{iGb})
                     warning('tGbPts not equal');
                 end
                 if ~isequal(struRef{iE}(iS).tGbNormal{iTwin}{iGb}, struCell{iE}(iS).tGbNormal{iTwin}{iGb})
                     warning('tGbNormal not equal');
                 end
                 if ~isequal(struRef{iE}(iS).tGbPtsiE{iTwin}{iGb}, struCell{iE}(iS).tGbPtsiE{iTwin}{iGb})
                     warning('tGbPtsiE not equal');
                 end
             end
             disp(['iE,iS,iTwin,iGb = ', num2str([iE,iS,iTwin,iGb])]);
