...
% grainDataPath = [uigetdir('C:\Users\chenz\Work\zhec\WE43_T6_C1_insitu_compression\Analysis_by_Matlab\Grain_1144_data_for_paper_ppt','Folder to save the grain data'),'\'];
dicPath = uigetdir('C:\Users\chenz\Work\zhec\WE43_T6_C1\SEM Data\stitched_DIC','pick DIC directory, which contains the stitched DIC data for each stop');
dicFiles = dir([dicPath,'\*.mat']);
dicFiles = struct2cell(dicFiles);
dicFiles = dicFiles(1,:)';

% looks like have to include this part to read the sample name.
[fileSetting,pathSetting] = uigetfile('C:\Users\chenz\Work\codes\DIC_Analysis\setting_for_real_samples\WE43_T6_C1_setting.mat','select setting file which contains sampleName, stopNames, FOVs, translations, etc');
load_settings([pathSetting,fileSetting],'sampleName','cpEBSD','cpSEM','sampleMaterial','stressTensor','strainPauses');

% load previous data and settings
saveDataPath = [uigetdir('C:\Users\chenz\Work\zhec\WE43_T6_C1\Analysis_2021_09','choose a path of the saved processed data, or WS, or etc.'),'\'];
saveDataPathInput = saveDataPath;
load([saveDataPath,sampleName,'_traceAnalysis_WS_settings.mat']);
if ~strcmpi(saveDataPath,saveDataPathInput)
    warning('Input saveDataPath is different from that saved in setting.mat file. Check files or code.');
    saveDataPath = saveDataPathInput;
end

% Load from the pre-labeled results: twinMap, sfMap, struCell.  (cToGbDistMap is omitted, as will no longer be used in this code)
% [confirmedLabelFile, confirmedLabelPath] = uigetfile('C:\Users\chenz\Work\zhec\DIC_Analysis\*.mat','select the results where twin identification was based on trace dir and strain');

% [twinGbIntersectionFile, twinGbIntersectionPath] = uigetfile('C:\Users\chenz\Work\zhec\DIC_Analysis\temp_results\*.mat','select the results for twin-grain boundary intersection');
...
[newVariantFile, newVariantFilePath] = uigetfile('C:\Users\chenz\Work\zhec\WE43_T6_C1\Analysis_2021_09\WE43_T6_C1_6_struCell_with_tGbVol.mat','select file for the most recent struCell');
...
%         traceDir = abs_schmid_factor(nss+1:nss+ntwin,3);
        ...
        eMap_local = eMap(indR_min:indR_max, indC_min:indC_max);  % This is for effective strain (eMap uses previous iE info !!!)
        ...
        if any(cellfun(@(x) ~isempty(x), struCell{iE}(iS).tGb)) % sum(struCell{iE}(iS).cTrueTwin(:))>0
            grain_twinned_TF = true;
        else
            grain_twinned_TF = false;
        end
            ...
            for iNb = 1:nNeighbors
                ...
                miso = calculate_misorientation_euler_d(euler,euler_nb,'hcp');
