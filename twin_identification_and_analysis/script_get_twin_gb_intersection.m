...
% Load cluster number maps (cleaned). Temporary disable.
...
for iS = 1:length(struCell{iE})     % e.g., iS=1253, ID_current=1390, ID=378
    ...
    if sum(struCell{iE}(iS).cTrueTwin(:))>0
        ...
        branchNumbered = one_pass_label_8(branch_tn);    % here should use 8-connectivity to label
        branchNumbered(~branch_tn) = 0;
