...
%
% Based on the older version, change the 'available deformation modes'?

clear;
addChenFunction;

save_dir = 'C:\Users\ZheChen\Desktop\New folder';  % dir to save the analysis result. Can copy from this later.
...
%                         end
                        ...
                        for iTwin=1:6
                            ...
                            % catch: if no ss available in neighbor
                            try
                                ...
                                T_local.mPrime(ir) = nan;
                                T_local.rank_mPrime(ir) = nan;
                                T_local.ssn_nb(ir) = nan;
                                T_local.SF_nb(ir) = nan;
                            end                            
                            try
                                ...
                                T_local.resB(ir) = nan;
                                T_local.rank_resB(ir) = nan;
                                T_local.ssn_nb_r(ir) = nan;
                                T_local.SF_nb_r(ir) = nan;
                            end
                            ...
                            try
                                ...
                                T_local.mPrime_wrtB(ir) = nan;
                                T_local.rank_mPrime_wrtB(ir) = nan;
                                T_local.ssn_nb_wrtB(ir) = nan;
                                T_local.SF_nb_wrtB(ir) = nan;
                            end
                            try
                                ...
                                T_local.resB_wrtB(ir) = nan;
                                T_local.rank_resB_wrtB(ir) = nan;
                                T_local.ssn_nb_r_wrtB(ir) = nan;
                                T_local.SF_nb_r_wrtB(ir) = nan;
                            end
                            ...
                            try
                                ...
                                T_local.mPrime_wrtT(ir) = nan;
                                T_local.rank_mPrime_wrtT(ir) = nan;
                                T_local.ssn_nb_wrtT(ir) = nan;
                                T_local.SF_nb_wrtT(ir) = nan;
                            end
                            try
                                ...
                                T_local.resB_wrtT(ir) = nan;
                                T_local.rank_resB_wrtT(ir) = nan;
                                T_local.ssn_nb_r_wrtT(ir) = nan;
                                T_local.SF_nb_r_wrtT(ir) = nan;
                            end
...
%                             % struCell{}().tGbVol is summarized in temp_scripts..., maybe no need to run at this time. 
...
%            label_map_with_ID(X_local,Y_local,ID_local,gcf,T_local.ID_neighbor(ind),'m');
            ...
            if plotTF==1
                ...
                saveas(gcf,fullfile(save_dir, ['iE_',num2str(iE),'_ID_',num2str(ID_current),'.tiff']));
            end
...
%            label_map_with_ID(X_local,Y_local,ID_local,gcf,T_local.ID_neighbor(ind),'m');
    ...
    %% Save
        ...
        save(fullfile(save_dir, [timeStr,'_twin_gb_summary_',num2str(iE),'.mat']), 'struCell','T','T2', 'lookupMa','lookupMb', ...
            ...
            '-v7.3');
        copyfile(fullfile(save_dir, [timeStr,'_twin_gb_summary_',num2str(iE),'.mat']), fullfile(save_dir,['twin_gb_summary_',num2str(iE),'.mat']));
...
for iE = 2:5
    ...
    switch iE
        case 2
            edmat_2 = edmat;
            save(fullfile(save_dir, ['twin_gb_summary_',num2str(iE),'.mat']),'edmat_2','x_dist', '-append','-v7.3');
        case 3
            edmat_3 = edmat;
            save(fullfile(save_dir, ['twin_gb_summary_',num2str(iE),'.mat']),'edmat_3','x_dist', '-append','-v7.3');
        case 4
            edmat_4 = edmat;
            save(fullfile(save_dir, ['twin_gb_summary_',num2str(iE),'.mat']),'edmat_4','x_dist', '-append','-v7.3');
        case 5
            edmat_5 = edmat;
            save(fullfile(save_dir, ['twin_gb_summary_',num2str(iE),'.mat']),'edmat_5','x_dist', '-append','-v7.3');
    end
...
if 0
    for iE = 2:5
        ...
        save(fullfile(save_dir, ['twin_gb_summary_',num2str(iE),'.mat']),'T','T2','-append');
    end
end
