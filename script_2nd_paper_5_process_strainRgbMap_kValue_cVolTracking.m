...
for ii = 1:length(iE_list)
    ...
    switch iE
        case 2
            titleStr = '\epsilon\fontsize{18}^G\fontsize{24} = -0.004';
        case 3
            titleStr = '\epsilon\fontsize{18}^G\fontsize{24} = -0.012';
        case 4
            titleStr = '\epsilon\fontsize{18}^G\fontsize{24} = -0.023';
        case 5
            titleStr = '\epsilon\fontsize{18}^G\fontsize{24} = -0.039';
    end
    % titleStr = 'Cluster ID';
    ...
    title(a,[titleStr,', Cluster ID'],'fontweight','normal');
%     title(c,'Cluster ID');
    ...
    c.Ticks = 1:n;
    set(c,'limits',[0.5,n-0.5]);
    title(a,[titleStr,', Cluster ID Cleaned'],'fontweight','normal'); 
%     title(c,'Cluster ID');
    set(a,'fontsize',28,'xTickLabel',{''},'yTickLabel',{''});
    axis equal;
...
ax = axes;
hold on;
plot(strainPauses(iE_list),volEvo,'--ro','linewidth',1.5,'markersize',8);
plot(strainPauses(iE_list),volEvoCleaned,'-rd','linewidth',1.5,'markersize',8);
plot(strainPauses(iE_list),volEvo_2,'--ko','linewidth',1.5,'markersize',8);
plot(strainPauses(iE_list),volEvoCleaned_2,'-kd','linewidth',1.5,'markersize',8);
xlabel('Global Uniaxial Strain, mm/mm');
xlabel('\fontsize{24}\epsilon\fontsize{12}^G');
ylabel('Cluster Size, pixels');
title('Cluster Size Evolution','fontweight','normal');
